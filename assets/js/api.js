...
const put = async (url, data, config = {}) => {
  return axiosApi.put(url, { ...data }, { ...config }).catch((e) => e.response);
};

const del = async (url, config = {}) => {
  return await axiosApi.delete(url, { ...config }).catch((e) => e.response);
};
...
//DASHBOARD
const HOME_DASHBOARD = "/refactory/dashboard.html";
...
const GET_BUSINESS_PERFORMANCE_ANALYSIS =
  "/reporters/business-performance-analysis";
...
const getDashboardBusinessSnapshot = (data) =>
  post(GET_BUSINESS_SNAPSHOT, data);
const getDashboardEmployeePerformance = (data) =>
  post(GET_EMPLOYEE_PERFORMANCE, data);
const getDashboardBusinessPerformanceAnalysis = (data) =>
  post(GET_BUSINESS_PERFORMANCE_ANALYSIS, data);
...
const f_auth = async () => {
  ...
  if (result.status == 201) {
    ...
    window.location.href = HOME_DASHBOARD;
  }
...
const setStorage = (data) => {
  localStorage.setItem("token", data["itoken"]);
  localStorage.setItem("profile", JSON.stringify(data["data"]));
  localStorage.setItem("usersList", JSON.stringify(data["userList"]));
};
...
const getCompanies = async () => {
  ...
  const result = data.map((el) => {
    return {
      ...
      status: el["flag"]? el["flag"]: '',
      integration: el["businessType"] ? el["businessType"]: '',
      vms: el["player"] ? el["player"] : '',
    };
  });
